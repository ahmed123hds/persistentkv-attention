...
B8 bimodal $G=4$ trace. The profiler replays instrumented kernels, so these
numbers are hardware-counter evidence rather than the source of wall-clock
speedups. PersistentKV decode reaches higher SM and memory-throughput
utilization than the profiled FlashInfer decode invocation, supporting the
claim that page-aware work decomposition exposes more useful device work. The
merge kernel is short but memory-heavy, matching the launch/merge overhead
concern in the B4 boundary case.
...
is not a full profiling campaign: the main five-seed rows are still timed
without Nsight replay, and datacenter GPUs should be profiled separately.
